...
export function createVideoClip({ mediaId, type, duration, width, height }) {
  ...
  return {
    ...
    // recorte manual (fracciones del original)
    ...
    mask: 'none',        // none | circle | roundrect | heart | star | triangle
...
export function createAudioClip({ mediaId, duration, start = 0, name = 'Audio' }) {
  return {
    ...
    volPoints: null, // curva de volumen dibujada a mano: [[segundo, 0..2], …]
  };
}
...
export function normalizeProject(p) {
  ...
  p.fadeIn = p.fadeIn ?? 0; p.fadeOut = p.fadeOut ?? 0;
  ...
  for (const c of p.tracks.audio) { c.muted = c.muted ?? false; c.fadeIn = c.fadeIn ?? 0; c.fadeOut = c.fadeOut ?? 0; c.duck = c.duck ?? false; c.volPoints = c.volPoints || null; }
  for (const c of p.tracks.text) {
    ...
    c.shadow = c.shadow ?? false; c.letterSpacing = c.letterSpacing ?? 0; c.vertical = c.vertical ?? false;
  }
  return p;
}
...
export function clipFilterString(clip) {
  ...
  switch (clip.filter) {
    ...
    case 'sepia': f += ' sepia(.7) contrast(1.05) brightness(1.02)'; break;
    case 'invert': f += ' invert(1)'; break;
    case 'lomo': f += ' contrast(1.35) saturate(1.4) sepia(.2) brightness(.95)'; break;
    case 'candy': f += ' saturate(1.6) hue-rotate(310deg) brightness(1.08) contrast(1.05)'; break;
    case 'moody': f += ' brightness(.9) contrast(1.18) saturate(.72) sepia(.2)'; break;
    case 'frost': f += ' brightness(1.1) saturate(.85) hue-rotate(175deg) contrast(.98)'; break;
  }
